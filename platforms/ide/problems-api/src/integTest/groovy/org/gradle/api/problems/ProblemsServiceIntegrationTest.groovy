    ...
    def "can throw a problem with a wrapper exception"() {
        ...
        buildFile """
            abstract class ProblemReportingTask extends DefaultTask {
                ...
                @TaskAction
                void run() {
                    problems.forCoreNamespace().throwing {
                        spec -> spec
                            .label("label")
                            .category("type")
                            .withException(new RuntimeException("test"))
        ...
        buildFile """
            abstract class ProblemReportingTask extends DefaultTask {
                ...
                void run() {
                    ...
                    problems.forCoreNamespace().rethrowing(exception) { it
                        .label("label")
                        .category("type")
                    }
                }
        ...
        buildFile """
            abstract class ProblemReportingTask extends DefaultTask {
                ...
                void run() {
                    try {
                        def exception = new RuntimeException("test")
                        problems.forCoreNamespace().throwing { spec -> spec
                            .label("inner")
                            .category("type")
                            .withException(exception)
                        }
                    } catch (RuntimeException ex) {
                        problems.forCoreNamespace().rethrowing(ex) { spec -> spec
                            .label("outer")
                            .category("type")
                        }
                    }
